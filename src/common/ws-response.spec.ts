// Handlery przekazywane do `wsRespond` są tu celowo `async` bez `await` —
// dokładnie tak wyglądają handlery gatewayów, które rzucają w tym samym ticku,
// a test sprawdza, że koperta łapie oba rodzaje błędów tak samo.
...
describe('wsRespond', () => {
  ...
  it('Prisma P2002 → CONFLICT bez tekstu Prismy w error', async () => {
    const result = await wsRespond(async () => {
      throw new Prisma.PrismaClientKnownRequestError(
        'Unique constraint failed on the fields: (`recipeId`)',
        {
          code: 'P2002',
          clientVersion: 'test',
        },
  ...
  it('zwykły Error → INTERNAL_ERROR, komunikat zamaskowany, requestId w logu', async () => {
    ...
    expect(errorLog).toHaveBeenCalledWith(
      expect.stringContaining(
        `weeklyPlans:upsertWeekSlot 500 INTERNAL_ERROR requestId=${requestId}: prisma boom with sql`,
      ),
      expect.any(String),
    );
  });
  ...
  it('rzut nie-Error też daje INTERNAL_ERROR', async () => {
    ...
    const result = await wsRespond(async () => {
      // Test rzutu nie-Error — to jest sedno przypadku, nie przeoczenie.
