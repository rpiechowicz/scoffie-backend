...
describe('mapError', () => {
  it('AppException przechodzi 1:1, bez logu dla 4xx', () => {
    const mapped = mapError(
      new AppException('NOT_HOUSEHOLD_MEMBER', 'nie należysz', HttpStatus.FORBIDDEN),
    );
    expect(mapped).toEqual({
      contract: { code: 'NOT_HOUSEHOLD_MEMBER', message: 'nie należysz', status: 403 },
      log: null,
    });
  });
  ...
  it('AppException przekazuje details', () => {
    const mapped = mapError(
      new AppException('VALIDATION_ERROR', 'zła lista', HttpStatus.BAD_REQUEST, ['a', 'b']),
    );
  ...
  it('AppException 5xx zostawia komunikat, ale loguje jako error', () => {
    const mapped = mapError(
      new AppException('COOKIDOO_SERVICE_UNAVAILABLE', 'Cookidoo leży', HttpStatus.SERVICE_UNAVAILABLE),
    );
    expect(mapped.contract).toMatchObject({ code: 'COOKIDOO_SERVICE_UNAVAILABLE', message: 'Cookidoo leży', status: 503 });
  ...
  it.each([
    ...
    [new HttpException('wolniej', HttpStatus.TOO_MANY_REQUESTS), 'TOO_MANY_REQUESTS', 429, 'wolniej'],
  ...
  it('ValidationPipe (tablica message) → VALIDATION_ERROR z details', () => {
    const error = new BadRequestException(['a must be longer', 'b is required']);
  ...
  it('503 z Nesta zostaje 503 z kodem SERVICE_UNAVAILABLE i logiem', () => {
    ...
    expect(mapped.contract).toEqual({ code: 'SERVICE_UNAVAILABLE', message: 'chwilowo', status: 503 });
    expect(mapped.log?.level).toBe('error');
  });

  it('InternalServerErrorException maskuje komunikat', () => {
    const mapped = mapError(new InternalServerErrorException('select * from users'));
    expect(mapped.contract).toEqual({ code: 'INTERNAL_ERROR', message: INTERNAL_ERROR_MESSAGE, status: 500 });
    expect(mapped.log).toMatchObject({ level: 'error', message: 'select * from users' });
  });
  ...
  ])('Prisma %s → %s %i, tekst Prismy tylko w logu', (prismaCode, code, status) => {
    const mapped = mapError(prismaError(prismaCode, { target: ['recipeId'] }));
    expect(mapped.contract).toMatchObject({ code, status });
    expect(mapped.contract.message).not.toContain('prisma');
    expect(mapped.log).toMatchObject({ level: 'warn', message: expect.stringContaining(prismaCode) });
    expect(mapped.log?.message).toContain('recipeId');
  });

  it('nieznany kod Prismy → INTERNAL_ERROR', () => {
    const mapped = mapError(prismaError('P2034'));
    expect(mapped.contract).toMatchObject({ code: 'INTERNAL_ERROR', status: 500, message: INTERNAL_ERROR_MESSAGE });
    expect(mapped.log?.level).toBe('error');
  });
  ...
  it.each([['string'], [undefined], [null], [42]])('rzut nie-Error (%p) nie wywraca mapera', (thrown) => {
  ...
  it('obiekt http-errors (body-parser) dostaje kod ze statusu', () => {
    expect(mapError({ statusCode: 413, message: 'too large' }).contract).toEqual({
      code: 'HTTP_ERROR',
      message: 'Nieprawidłowe żądanie.',
      status: 413,
    });
    expect(mapError({ statusCode: 400, type: 'entity.parse.failed' }).contract.code).toBe('BAD_REQUEST');
  });
});

describe('toHttpBody', () => {
  it('składa body bez statusCode i bez pustego details', () => {
    expect(toHttpBody({ code: 'NOT_FOUND', message: 'nie ma', status: 404 }, 'req-1')).toEqual({
      code: 'NOT_FOUND',
      message: 'nie ma',
      requestId: 'req-1',
    });
    expect(
      toHttpBody({ code: 'VALIDATION_ERROR', message: 'x', status: 400, details: ['x'] }, 'req-2'),
    ).toEqual({ code: 'VALIDATION_ERROR', message: 'x', details: ['x'], requestId: 'req-2' });
  });
});
