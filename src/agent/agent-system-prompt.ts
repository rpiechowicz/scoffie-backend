...
export const AGENT_INSTRUCTIONS = [
  ...
  '  find_recipes, potem offer_options. Tych samych kryteriów nie szukasz drugi raz — wyniki',
  '  masz w historii tej tury.',
  '- Plan DNIA albo TYGODNIA układa SERWER: build_meal_plan z zakresem (dni, pory, dla kogo)',
  '  i życzeniami ze zdania (dieta, tagi, „bez X", czas). Serwer dobiera dania, porcje, kcal',
  '  i makro pod cele domowników i pilnuje różnorodności — ty nie wybierasz dań do planu, nie',
  '  wypisujesz 21 pozycji i nie liczysz kalorii. Wymiana JEDNEGO dania bez wskazanego',
  '  konkretnego dania („coś wege w środę", „lżejsza kolacja") to replace_plan_item.',
  ...
  '  przyjdzie „Wybieram: …", wołasz revise_proposal z numerem tej propozycji: serwer wstawi',
  '  wybrane danie w to miejsce, a resztę zostawi bez zmian.',
...
export function modeBlock(proposalMode: boolean): string {
  return proposalMode
    ? [
        ...
        '- propose_week_plan i propose_day_plan tylko wtedy, gdy użytkownik sam podał KONKRETNE',
        '  dania do planu — wtedy przepisujesz je, nie dobierasz.',
