...
| 1. Poprawność, stan, koszty | **DONE** — czeka na review; 2 decyzje ops w §9 raportu | `reports/01-correctness-state-costs.md` |
| 2. Server-side planner | **READY** (start dopiero po akceptacji Etapu 1) | `reports/02-server-side-planner.md` |
...
- Limit `/auth/refresh` per rodzina tokenów vs obecny hasz tokenu + bezpiecznik IP.
...
- migracja `20260926120000_ksiega_per_wywolanie`; testy: unit 3309/3309, e2e 543/548
  (5 porażek środowiskowych, identycznych na `1c265a3`).
...
**Wykonawca zatrzymany — czeka na review Etapu 1.**
