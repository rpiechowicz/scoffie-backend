...
| Etap | Status | Raport |
|---|---|---|
| 0. Baseline | **PARTIAL** — harness i pomiary lokalne gotowe, brak płatnego przebiegu (czeka na zgodę) | `reports/00-baseline.md` |
| 1. Poprawność, stan, koszty | WAITING — na decyzję Rafała: najpierw płatny baseline czy start bez niego | `reports/01-correctness-state-costs.md` |
...
- Płatny baseline nowego przepływu (Etap 0) — wariant i zgoda na koszt.
...
`reports/00-baseline.md` (26.09.2026) — Etap 0 **PARTIAL**:
...
- **brak płatnego przebiegu** — nowy przepływ na żywym modelu niezmierzony.

Czeka na decyzję Rafała (§9 raportu): zgoda na płatny baseline (wariant
minimalny ~$2–3, pełny ~$3–6 — szacunki) i czy Etap 1 może ruszyć bez niego.
