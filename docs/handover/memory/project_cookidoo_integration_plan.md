...
1. Nowe repo `scoffie-cookidoo` (FastAPI + cookidoo-api, bezstanowy, `X-Internal-Token`, cache sesji per household).
...
**PRODUKCJA WDROŻONA 2026-08-26:** Railway projekt `soothing-celebration`: serwis `scoffie-cookidoo` postawiony przez `railway up` (GitHub app nie miała dostępu do repo — `railway add --repo` dawał Unauthorized; kod też na github.com/rpiechowicz/scoffie-cookidoo, main+develop), **PORT=8000 przypięty jawnie** (Railway wstrzykiwał 8080, a COOKIDOO*SERVICE_URL celuje w :8000). API `scoffie-backend` deployuje z gałęzi `main`; env-y COOKIDOO*\* ustawione przez `railway variables --skip-deploys` PRZED merge (fail-fast na kluczu). Weryfikacja przez /ops/health po commit SHA (grep po logach łapie historyczne deploye!). Prod DB: import polish-classics-11 (bot „Recipe Import Bot", jedyne gospodarstwo „Home"), nutrition 135 wpisów, recompute 89/89, Leczo 1:1 zweryfikowane. Plik z sekretami usunięty — wartości tylko w Railway Variables. Prod-content-update robi się przez `docker exec -e DATABASE_URL=<DATABASE_PUBLIC_URL z serwisu Postgres> scoffie-api npx tsx scripts/...`.

**Status 2026-08-26 — WSZYSTKIE 4 FAZY ZAIMPLEMENTOWANE:** repo `scoffie-cookidoo` (develop, efdf9dd; cookidoo-api==0.17.2, auth przez cookies w aiohttp — cache sesji trzyma żywe ClientSession per household); backend `feat/integracja-cookidoo` (38028a1, zweryfikowane curl E2E ze złym hasłem aż do prawdziwego Cookidoo); iOS `feat/integracje-cookidoo` (8b14bc9) i `feat/thermomix-przepisy` (ad8de13, stackowany), oba budują się xcodebuildem. Leczo w dev DB podlinkowane do prawdziwego id **r56899**. Zostało: test E2E z prawdziwymi credsami użytkownika (Ustawienia w apce), PR-y do develop, deploy Railway (kolejność: Python → API → iOS). Powiązane: [[project-scoffie-stack]], [[project-branching-develop]], [[project-mac-resources-exhausted]].
