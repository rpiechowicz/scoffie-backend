...
**How to apply:** zanim zaczniesz szukać winy w kodzie albo w zależnościach, sprawdź `df -h /System/Volumes/Data` i `sysctl vm.swapusage`. Docker: wolumen `scoffie-backend_pgdata` trzyma bazę przepisów — nigdy nie czyść Dockera z flagą `--volumes`. Powiązane: [[project-recipe-macro-convention]], [[project-scoffie-stack]].
...
`git status`/`git add`/`git fetch` w `scoffie-backend` trwają 3–4 min przy 0,02 s CPU (czyste I/O,
