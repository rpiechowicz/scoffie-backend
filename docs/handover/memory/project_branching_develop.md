...
Oba repozytoria Scoffie — `scoffie-ios` i `scoffie-backend` — mają gałąź integracyjną `develop` i to z niej zakłada się nowe branche. Lokalne kopie refów potrafią być mocno przeterminowane (backendowy `master` wyglądał na starszy o cztery miesiące, a `origin/develop` w ogóle nie istniał lokalnie, dopóki nie zrobiłem `git fetch --prune`).
