...
- `B/.env.example:9` `AUTH_DEV_LOGIN_ENABLED=true`; `B/DEPLOYMENT.md:23` lists `AUTH_DEV_LOGIN_ENABLED=true` in the *minimum production* set.
...
## F10 — P1 — `SAFE_MIGRATE_REBUILD_DB` drops the production schema on *every* container start for as long as the env var stays set
**Evidence**
