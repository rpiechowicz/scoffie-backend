...
| 11 | Cache states | **warm** = everything hit, only new tail written; **cold-user** = shared prefix hit, household+history written (realistic "resume a conversation after TTL"); **cold-all** = nothing hit. A *brand-new* conversation has history=0, so its cold cost ≈ warm + 676-token household write (≈$0.003 on Sonnet) — cold only hurts when *resuming* long conversations | derived |
